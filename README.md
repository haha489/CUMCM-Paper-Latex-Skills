...
**以作者完成的模型、代码与结果为依据，系统还原建模逻辑、组织论证与图表，将分散的研究成果整理为结构严谨、结果可核、风格统一且可直接编译的中文 LaTeX 数模论文。**
...
不少数模 skills 同时承担读题、建模、编程、求解、绘图和论文写作。这样的“大而全”流程看似方便，但每个环节能够投入的上下文和检查力度有限：模型容易停留在通用套路，论文也容易变成算法名称、公式和结果的简单拼接。不同任务反复生成后，章节风格、公式记号、表格格式和图片质量也很难保持一致。

实际竞赛中的建模往往需要作者持续参与。作者会根据中间结果修改假设、调整参数、比较方案，并把对题目背景的理解和主观判断加入模型。这些交互本身就是研究过程的一部分，直接让 AI 一次性给出模型与答案，通常难以替代作者对问题的判断，也容易让后续论文建立在未经确认的结果上。

因此，本项目把工作范围集中在“已有成果到正式论文”这一段。模型选择、代码运行、关键结果和主要图片由作者掌握；skill 读取这些真实成果，负责还原数学逻辑、组织论证、统一全文表达、完善 LaTeX 排版并核查结果来源。清晰的职责划分让 AI 把上下文和精力集中到论文质量上，也让作者保留对研究内容的最终控制。

| 常见问题 | 本项目的处理方式 |
|---|---|
| 解题与写作全部塞进一次生成，模型和论文都缺少深入检查 | 在模型和结果已经基本完成后启动，集中处理代码—数学—结果—文字之间的一致性 |
| AI 直接决定模型与答案，作者难以逐步加入经验和判断 | 作者参与建模、运行与调整，skill 依据作者确认的成果成稿，不静默替换研究方案 |
| 每次生成的论文风格不稳定，公式、表格、图片和章节格式各不相同 | 提供固定的 LaTeX 工程、分章节写法、符号与结果管理方式，并在编译后逐页检查 |
| 一体化生成的图片往往缺少针对性，同时制作大量图片会分散论文写作重点 | 先盘点总体流程、数据侧写和逐问结果图；作者可对最重要的单图继续精修，论文保留统一图题与解读 |
| 发现缺口后要么直接忽略，要么让 AI 自动扩展大量新工作 | 单独生成 `论文完善建议.md`，说明新增内容的依据、位置与价值，由作者选择是否继续 |
| 摘要堆叠算法名或重复正文，有限篇幅没有呈现真正的工作量 | 在全文完成后压缩摘要，用尽可能少的文字交代关键处理、模型机制和核心答案，再由作者作最终加工 |

这种分工也避免写作任务不断滑向重新训练、批量实验和无边界的补图。若现有成果中已经有可靠数据和图表，skill 可以直接整理、重绘并纳入论文；需要新增研究或精修关键图片时，则先给出具体建议，让作者决定投入方向。
